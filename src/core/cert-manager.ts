...
function expandHomeRelative(input: string, home: string): string {
...
function resolveMittensHomeDir(env: NodeJS.ProcessEnv): string {
  const explicit = env.OPENCLAW_HOME?.trim();
  return explicit ? expandHomeRelative(explicit, homedir()) : homedir();
}

// Resolve the Mittens state-dir root. Resolution order:
//   1. MITTENS_STATE_DIR (tilde/relative resolved)
//   2. OPENCLAW_STATE_DIR (plugin compat, tilde/relative resolved)
//   3. dirname(OPENCLAW_CONFIG_PATH) — custom config path keeps certs co-located
//   4. <home>/.mittens
// Stored under the state dir rather than tmpdir() for stability — tmpdir
// contents can be wiped on macOS between reboots, causing unnecessary cert regen.
export function resolveMittensStateDir(env: NodeJS.ProcessEnv = process.env): string {
  const home = resolveMittensHomeDir(env);
  const stateDir = (env.MITTENS_STATE_DIR ?? env.OPENCLAW_STATE_DIR)?.trim();
  if (stateDir) {
    return expandHomeRelative(stateDir, home);
  }
  const configPath = env.OPENCLAW_CONFIG_PATH?.trim();
  if (configPath) {
    return dirname(expandHomeRelative(configPath, home));
  }
  return join(home, ".mittens");
}

export { resolveMittensStateDir as resolveInteractiveProxyStateDir };

const CERT_DIR = join(resolveMittensStateDir(), "proxy-certs");
...
// Fast leaf-cert-only generator for dynamic SNI. Reuses the existing CA
// (which must already exist from a prior ensureCerts call). Caches per hostname.
const sniCertCache = new Map<string, { key: Buffer; cert: Buffer }>();

export function ensureLeafCert(hostname: string): { key: Buffer; cert: Buffer } | null {
  const cached = sniCertCache.get(hostname);
  ...
  const leafSlug = hostname.replace(/[^a-zA-Z0-9.-]/g, "_");
  const leafCertPath = join(CERT_DIR, `leaf-${leafSlug}.crt`);
  const leafKeyPath = join(CERT_DIR, `leaf-${leafSlug}.key`);

  const openssl = findOpenssl();

  if (existsSync(leafCertPath) && existsSync(leafKeyPath) && isCertValid(openssl, leafCertPath)) {
    const result = { key: readFileSync(leafKeyPath), cert: readFileSync(leafCertPath) };
    sniCertCache.set(hostname, result);
    return result;
  }

  // Generate new leaf cert
  const csrPath = join(CERT_DIR, `leaf-${leafSlug}.csr`);
  const extPath = join(CERT_DIR, `leaf-${leafSlug}.ext`);

  try {
    runOpenssl(openssl, ["genrsa", "-out", leafKeyPath, "2048"]);
    runOpenssl(openssl, ["req", "-new", "-key", leafKeyPath, "-subj", `/CN=${hostname}`, "-out", csrPath]);
    ...
    runOpenssl(openssl, [
      ...
      "-CAcreateserial", "-out", leafCertPath, "-days", CERT_DAYS, "-sha256", "-extfile", extPath,
    ]);
  } catch {
    return null;
  }
...
export function ensureCerts(targetHost: string = "api.anthropic.com"): CertPaths {
  const leafSlug = targetHost.replace(/[^a-zA-Z0-9.-]/g, "_");
  const caPath = join(CERT_DIR, "ca.crt");
  const caKeyPath = join(CERT_DIR, "ca.key");
  const leafCertPath = join(CERT_DIR, `leaf-${leafSlug}.crt`);
  const leafKeyPath = join(CERT_DIR, `leaf-${leafSlug}.key`);
