...
import { parseAdapter } from "./cli-args.js";
...
const TTY_SPOOF_PATH = fileURLToPath(new URL("./tty-spoof.cjs", import.meta.url)).replace(/\\/g, "/");

function extractSessionId(args: string[], adapter?: import("../adapters/types.js").CliAdapter): string {
  const flags = new Set(["--session-id", "--resume"]);
  if (adapter?.session?.sessionArg) flags.add(adapter.session.sessionArg);
  for (let i = 0; i < args.length - 1; i++) {
    if (flags.has(args[i]!)) return args[i + 1] ?? "";
  }
  // Check {sessionId} template args
  if (adapter?.session?.resumeArgs) {
    for (let i = 0; i < args.length; i++) {
      for (const tmpl of adapter.session.resumeArgs) {
        if (tmpl.includes("{sessionId}") && args[i] === tmpl.replace("{sessionId}", args[i]!)) {
          return args[i]!;
        }
      }
    }
...
const DEBUG =
  process.env["MITTENS_DEBUG"] === "1" ||
  process.env["OPENCLAW_INTERACTIVE_PROXY_DEBUG"] === "1";
...
async function main(): Promise<void> {
  const { adapter: adapterId, rest: rawArgs } = parseAdapter(process.argv.slice(2));
  const adapter = getAdapter(adapterId);
  let claudeArgs = rawArgs;

  dbg(`starting adapter=${adapterId}, args:`, redactArgsForLog(claudeArgs).join(" "));
  ...
  // Inject adapter-specific flags BEFORE user args. For adapters like Cursor
  // where injectFlags contain the binary path (wsl -- cursor-agent -p ...),
  // user args must come after the injected flags.
  if (adapter.injectFlags?.length) {
    claudeArgs = [...adapter.injectFlags, ...claudeArgs];
  }

  let certs: Awaited<ReturnType<typeof ensureCerts>> | null = null;
  let proxy: Awaited<ReturnType<typeof startMitmProxy>> | null = null;
  let redirect: RedirectHandle | null = null;

  if (adapter.mode === "mitm") {
    const targetHost = adapter.target?.host ?? "api.anthropic.com";
    try {
      certs = ensureCerts(targetHost);
      dbg("certs ready:", certs.caPath);
    } catch (e) {
      process.stderr.write(
        `[mittens] cert generation failed: ${e instanceof Error ? e.message : String(e)}\n`,
      );
    ...
    try {
      proxy = await startMitmProxy(certs, adapter.target ? {
        host: adapter.target.host,
        port: adapter.target.port,
      ...
      process.stderr.write(
        `[mittens] proxy start failed: ${e instanceof Error ? e.message : String(e)}\n`,
      );
    ...
    process.stderr.write(
      `[mittens] MITM proxy active on 127.0.0.1:${proxy.connectPort}` +
      ` — ${targetHost} traffic intercepted\n`,
    );

    // Kernel-level port redirect for gRPC adapters that ignore HTTPS_PROXY
    if (adapter.target?.needsKernelRedirect && proxy.h2Port > 0) {
      flushStaleRules();
      const destIPs = resolveIPv4(targetHost);
      if (destIPs.length > 0) {
        try {
          redirect = await setupRedirect({ targetPort: proxy.h2Port, destIPs });
          dbg("kernel redirect active:", destIPs.join(","), "-> h2Port", proxy.h2Port);
        } catch (e) {
          process.stderr.write(
            `[mittens] kernel redirect failed (falling back to HTTPS_PROXY only): ` +
            `${e instanceof Error ? e.message : String(e)}\n`,
          );
        }
      } else {
        dbg("no IPs resolved for", targetHost, "— skipping kernel redirect");
      }
    }
  } else {
    dbg("stdout-pipe mode, no MITM proxy");
  }
  ...
  const stopProxy = () => {
    if (redirect) redirect.cleanup().catch(() => {});
    if (proxy) proxy.stop().catch(() => {});
    cleanupOverflowFile();
    if (heartbeatTimer) {
  ...
  const sessionId = extractSessionId(claudeArgs, adapter);
  ...
  if (adapter.envOverrides) {
    for (const [k, v] of Object.entries(adapter.envOverrides)) {
      cliEnv[k] = v;
    }
  }
  if (proxy && certs) {
  ...
  if (adapter.needsTtySpoof) {
    ...
    cliEnv.NODE_OPTIONS = (cliEnv.NODE_OPTIONS ? cliEnv.NODE_OPTIONS + " " : "") + "--use-system-ca";
  }

  let cliBinary = (adapter.binaryEnvVar ? process.env[adapter.binaryEnvVar] : undefined)
    || (adapter.id === "claude" ? process.env["OPENCLAW_INTERACTIVE_CLAUDE_BINARY"] : undefined)
    || adapter.binary;
  // Bun's spawn on Windows doesn't resolve bare names through PATHEXT like a
  // shell does, so `gemini` fails to find `gemini.cmd`. Use `which` to resolve.
  if (process.platform === "win32" && !cliBinary.includes("/") && !cliBinary.includes("\\")) {
    try {
      const resolved = execFileSync("where", [cliBinary], { stdio: "pipe", encoding: "utf8" }).trim().split(/\r?\n/)[0];
      if (resolved) cliBinary = resolved;
    } catch {}
  }
  dbg(`spawning ${adapter.name} binary:`, cliBinary);
  // stdout: "ignore" for MITM mode (we read from the proxy tap), "pipe" for stdout mode
  const claude = spawn(cliBinary, claudeArgs, {
    env: cliEnv,
    stdio: ["pipe", adapter.mode === "stdout" ? "pipe" : "ignore", "inherit"],
  ...
  const waitForSessionWriteThenKill = (): void => {
    const sessionPath = adapter.hasSessionFiles ? claudeSessionFilePath(sessionId) : null;
  ...
  // Shared handler for normalized Claude-format JSONL lines. Both MITM and
  // stdout-pipe modes converge here after normalization. Each line is a
  // complete JSON record ({type:"stream_event",event:{...}} or {type:"result",...}).
  function handleNormalizedLine(line: string): void {
    let parsed: Record<string, unknown>;
    try { parsed = JSON.parse(line) as Record<string, unknown>; } catch { return; }

    const outerType = parsed.type as string;

    // "result" records from normalizers — emit directly and trigger kill
    if (outerType === "result") {
      ...
      emit(line);
      waitForSessionWriteThenKill();
      return;
    }

    // "init" records — emit directly
    if (outerType === "init") {
      emit(line);
      return;
    }

    // Everything else should be stream_event
    if (outerType !== "stream_event") { emit(line); return; }
    ...
    if (eventType === "message_stop" && turnActive) {
      turnActive = false;
      if (currentStopReason !== "end_turn") return;
      if (resultEmitted || intentionalKill) return;
      resultEmitted = true;
  ...
  // ── MITM mode: proxy tap → optional normalizer → handler ──
  if (proxy) {
    proxy.onEvent((evt) => {
      const eventType = evt.type as string;
      ...
      if (eventType === "api_error") {
        ...
        dbg("api_error event, status:", status, "exiting cleanly");
        intentionalKill = true;
      ...
      // Adapters with normalizers: transform raw events to Claude JSONL
      if (adapter.normalizeEvent) {
        const lines = adapter.normalizeEvent(evt);
        for (const l of lines) handleNormalizedLine(l);
        return;
      }
      ...
      if (requestType === "auxiliary") return;
  ...
  // ── Stdout-pipe mode: read CLI stdout → normalizer → handler ──
  if (adapter.mode === "stdout" && claude.stdout) {
    let stdoutBuf = "";
    ...
    claude.stdout.on("data", (chunk: string) => {
      ...
      while (nl !== -1) {
        const rawLine = stdoutBuf.slice(0, nl).trim();
        stdoutBuf = stdoutBuf.slice(nl + 1);
        if (!rawLine) { nl = stdoutBuf.indexOf("\n"); continue; }
        if (adapter.normalizeEvent) {
          const lines = adapter.normalizeEvent({ _rawLine: rawLine });
          for (const l of lines) handleNormalizedLine(l);
        } else {
          handleNormalizedLine(rawLine);
        }
